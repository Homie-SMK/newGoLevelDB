...
type version struct {
	...
	// cScore < 1 means compaction is not strictly needed. These fields
	// are initialized by computeCompaction()

	// cLevel is the level which has highest cScore
	// among levels having cScore equal to or larger than 1
	cLevel int

	// denotes highest cScore
	cScore float64
...
func (v *version) spawn(r *sessionRecord, trivial bool) *version {
	//SM
	//fmt.Println("SM: spawn entered")
	//fmt.Println("SM:db_path@", reflect.ValueOf(v.s.stor.Storage).Elem().Field(0), ":spawn entered:version_id@", v.s.stVersion.id)
...
func (v *version) computeCompaction() {
	//SM
	//fmt.Println("SM:db_path@", reflect.ValueOf(v.s.stor.Storage).Elem().Field(0), ":computeCompaction entered:version_id@", v.s.stVersion.id)
	v.s.log("SM:db_path@", reflect.ValueOf(v.s.stor.Storage).Elem().Field(0), ": computeCompaction entered")
	...
	//SM
	//fmt.Println("SM:cLevel@", v.cLevel, "@cScore@", v.cScore)

	for level := range v.levels {
		if level != bestLevel && 1 <= scores[level] && scores[level] <= bestScore {
			v.cLevels[level] = level
			v.cScores[level] = scores[level]
			//fmt.Println("SM:cLevels@", v.cLevels[level], "@cScores@", v.cScores[level])
		}
	}

	//SM
	v.s.logf("SM:db_path@%s:version@stat F·%v S·%s%v Sc·%v", reflect.ValueOf(v.s.stor.Storage).Elem().Field(0), statFiles, shortenb(int(statTotSize)), statSizes, statScore)
	v.s.log("SM:db_path@", reflect.ValueOf(v.s.stor.Storage).Elem().Field(0), ": end of computeCompaction")

	//fmt.Printf("SM:db_path@%s:version@stat F·%v S·%s%v Sc·%v\n", reflect.ValueOf(v.s.stor.Storage).Elem().Field(0), statFiles, shortenb(int(statTotSize)), statSizes, statScore)
	//fmt.Println("SM:db_path@", reflect.ValueOf(v.s.stor.Storage).Elem().Field(0), ": end of computeCompaction")
}
...
func (p *versionStaging) commit(r *sessionRecord) {
	//SM
	//fmt.Println("SM:db_path@", reflect.ValueOf(p.base.s.stor.Storage).Elem().Field(0), ":commit entered:version_id@", p.base.s.stVersion.id)
...
func (p *versionStaging) finish(trivial bool) *version {
	//SM
	//p.base.s.log("SM: finish entered")
	//fmt.Println("SM:db_path@", reflect.ValueOf(p.base.s.stor.Storage).Elem().Field(0), ":finish entered:version_id@", p.base.s.stVersion.id)
	p.base.s.log("SM:db_path@", reflect.ValueOf(p.base.s.stor.Storage).Elem().Field(0), ": finish entered")
