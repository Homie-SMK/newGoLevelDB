...
func newSession(stor storage.Storage, o *opt.Options) (s *session, err error) {
	...
	s.logf("SM:%s: newSession created: /leveldb/db_compaction", reflect.ValueOf(s.stor.Storage).Elem().Field(0))
	return
}
...
func (s *session) commit(r *sessionRecord, trivial bool) (err error) {
	//SM
	//s.log("SM: commit entered")
	//fmt.Println("SM:db_path@", reflect.ValueOf(s.stor.Storage).Elem().Field(0), ":commit entered:version_id@", s.stVersion.id)
...
/*
// Commit session; need external synchronization.
func (s *session) commit_m(r *sessionRecord, trivial bool) (err error) {
	//SM
	//s.log("SM: commit entered")
	fmt.Println("SM:db_path@", reflect.ValueOf(s.stor.Storage).Elem().Field(0), ":commit entered:version_id@", s.stVersion.id)
*/
