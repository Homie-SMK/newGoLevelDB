...
func (s *session) pickCompaction() []*compaction {
	//SM
	s.logf("SM:db_path@%s:pickCompaction entered:version_id@%d:/leveldb/session_compaction.go", reflect.ValueOf(s.stor.Storage).Elem().Field(0), s.stVersion.id)
	//fmt.Println("SM:dbpath@", reflect.ValueOf(s.stor.Storage).Elem().Field(0), ": pickCompaction entered")
	...
	if v.cScore >= 1 {
		...
		if p := atomic.LoadPointer(&v.cSeek); p != nil {
			...
			fmt.Println("SM:v.release() in pickCompaction")
			v.release()
			return nil
		}
	}
...
func newCompaction(s *session, v *version, sourceLevel int, t0 tFiles, typ int) *compaction {
	//SM
	//s.log("SM: newCompaction entered")
	fmt.Println("SM:dbpath@", reflect.ValueOf(s.stor.Storage).Elem().Field(0), ": newCompaction entered: /leveldb/db_compaction.go")
...
func (c *compaction) expand() {
	//SM
	//c.s.log("SM: expand entered")
	//fmt.Println("SM:dbpath@", reflect.ValueOf(c.s.stor.Storage).Elem().Field(0), ": expand entered")
	...
	if len(t1) > 0 {
		...
		if len(exp0) > len(t0) && t1.size()+exp0.size() < limit {
			...
			if len(exp1) == len(t1) {
				//SM
				c.s.logf("SM:dbpath@%s:table@compaction expanding L%d+L%d (F·%d S·%s)+(F·%d S·%s) -> (F·%d S·%s)+(F·%d S·%s)",
					reflect.ValueOf(c.s.stor.Storage).Elem().Field(0), c.sourceLevel, c.sourceLevel+1, len(t0), shortenb(int(t0.size())), len(t1), shortenb(int(t1.size())),
	...
	//SM
	c.s.logf("SM:dbpath@%s:sourceLevel:%v@imin:%v@imax:%v", reflect.ValueOf(c.s.stor.Storage).Elem().Field(0), c.sourceLevel, c.imin, c.imax)
	c.s.logf("SM:dbpath@%s:end of expand", reflect.ValueOf(c.s.stor.Storage).Elem().Field(0))
}
