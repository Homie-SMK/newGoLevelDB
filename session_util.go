...
func (s *session) setVersion(r *sessionRecord, v *version) {
	//SM
	//fmt.Println("SM:db_path@", reflect.ValueOf(s.stor.Storage).Elem().Field(0), "setVersion entered")
...
func (s *session) newManifest(rec *sessionRecord, v *version) (err error) {
	//SM
	//fmt.Println("SM:db_path@", reflect.ValueOf(s.stor.Storage).Elem().Field(0), "newManifest entered")
	...
	w, err := jw.Next()
	if err != nil {
		fmt.Println("SM: jw.Next failed")
		return
	}
	err = rec.encode(w)
	if err != nil {
		fmt.Println("SM: rec.encode(w) failed")
		return
	}
	err = jw.Flush()
	if err != nil {
		fmt.Println("SM: jw.Flush() failed")
		return
	}
	if !s.o.GetNoSync() {
		err = writer.Sync()
		if err != nil {
			fmt.Println("SM: writer.Sync() failed")
			return
		}
	}
	err = s.stor.SetMeta(fd)
	if err != nil {
		fmt.Println("SM: s.stor.SetMeta(fd) failed")
		return
	}
	return
}
...
func (s *session) flushManifest(rec *sessionRecord) (err error) {
	//fmt.Println("SM: flushManifest entered")
	s.fillRecord(rec, false)
	w, err := s.manifest.Next()
	if err != nil {
		fmt.Println("SM:s.manifest.Next() failed")
		return
	}
	err = rec.encode(w)
	if err != nil {
	...
	if err != nil {
		fmt.Println("SM:s.manifest.Flush() failed")
		return
	}
	if !s.o.GetNoSync() {
		err = s.manifestWriter.Sync()
		if err != nil {
			fmt.Println("SM:s.manifestWriter.Sync() failed")
			return
		}
	}
	s.recordCommited(rec)
