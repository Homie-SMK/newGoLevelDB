...
#df_no_comp = df[df['Compaction'] == 'NoCompaction']
#df_single_comp = df[df['Compaction'] == 'Normal']
#df_concurrent_comp = df[df['Compaction'] == 'Concurrent']

# Create the scatter plot
fig = px.scatter(df, x='BlockNumber', y='GetTime', color='Compaction', color_discrete_sequence=['blue', 'red', 'green'], opacity=0.7)

# Add labels and title
fig.update_layout(xaxis_title='INDEX', yaxis_title='GET LATENCY (MS)', title='ORIGINAL LEVELDB')
...
fig.update_xaxes(range=[7047333, 8000000])
fig.update_yaxes(range=[0, 300])

pio.write_image(fig, './original_get_latency_scatterplot_2.jpeg', scale=6, width=700, height=370)
